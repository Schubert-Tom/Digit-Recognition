...
cnn = CNN()
cnn.load_state_dict(torch.load('./data/cnn_model.pth'))
cnn.eval()

dnn = DNN()
dnn.load_state_dict(torch.load('./data/dnn_model.pth'))
dnn.eval()
##
def main():
    ...
    if st.sidebar.button("Guess the number!"):
        ...
        forward_propagate_cnn(converted_canvas_img_28x28)
        forward_propagate_dnn(converted_canvas_img_28x28)
...
def forward_propagate_cnn(img:Tensor):
    #[batch_size, channels, height, width] --> Tensor Should look like this 
    # forward through neural network.
    output=cnn(img) 
    _,pred_cnn=torch.max(output,1)
    # Print predict
    st.write("Convolutional Neural Network Prediction: " + str(pred_cnn.item()))    
    # training_img = sc.fit_transform(training_img)
    #st.subheader("Number Prediction")
    #st.write("Predicted digit is : " + str(predicted_digit))

def forward_propagate_dnn(img:Tensor):
    #[batch_size, channels, height, width] --> Tensor Should look like this
    print(img.shape)
    # Transform to Column Vector with batch dimension
    img = img.view(img.shape[0], -1) 
    # forward through neural network.
    output=dnn(img) 
    _,pred_dnn=torch.max(output,1)
    # Print predict
    st.write("Deep Neural Network Prediction: " + str(pred_dnn.item()))    
    # training_img = sc.fit_transform(training_img)
    #st.subheader("Number Prediction")
    #st.write("Predicted digit is : " + str(predicted_digit))
...
def display_canvas():
    ...
    st.subheader("Guessing with a CNN!")
